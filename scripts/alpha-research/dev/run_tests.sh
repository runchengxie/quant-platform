...
run_ty() {
  ...
  if [[ "${ALPHA_RESEARCH_NO_PROJECT_TOOLS:-0}" == "1" ]]; then
...
case "$mode" in
  all | fast | unit)
    ...
    ;;
  coverage)
    exec uv run python -m pytest --cov=alpha_research --cov-report=term-missing "$@"
    ;;
  lint)
    run_ruff check . "$@"
    ;;
  ...
  format | format-all)
    ...
    ;;
  maintainability)
    uv run --extra dev python scripts/alpha-research/dev/maintainability_metrics.py --ratchet "$@"
    ;;
  -h | --help | help)
    usage
    ;;
