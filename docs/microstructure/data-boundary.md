...
本仓库消费 `quant-market-data-platform` 已发布的市场数据。通用市场数据资产由 `quant-market-data-platform` 持有。
...
`ticknet` 通过已发布文件、schema、receipt 或本仓显式 adapter 接入跨仓输入。可复用的通用清洗规则回到 `quant-market-data-platform`。只服务于模型输入的归一化、窗口、特征和标签逻辑留在本仓。
...
- 项目 runtime dependencies 禁止声明 `quant-market-data-platform`、`tushare` 和 `rqdatac`。
