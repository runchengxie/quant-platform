...
QP 结果的 `diagnostics.constraint_residuals` 按约束名记录各暴露边界的有符号剩余量。下界使用约束名，上界使用 `<name>.upper`，负值表示违反边界。求解失败且等权回退也不可行时，错误消息会列出等权组合违反的暴露或权重边界。成功结果沿用 `portfolio_optimization_result.v1`，不增加诊断字段。

当前没有报告等式约束的秩或自由度。重复约束可以用秩分析识别，但秩和自由度本身不能判断二次目标函数、盒约束及其他暴露边界共同作用后的解是否唯一。现有调用方需要的是可行组合和命名残差，暂不扩大结果契约。
