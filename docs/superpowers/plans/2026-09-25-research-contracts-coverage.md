...
**实施记录（2026-09-25）：**已由 [PR #47](https://github.com/runchengxie/quant-platform/pull/47) 完成。契约测试直接覆盖已安装包，`contracts-coverage` 对该包设置 80% 门槛，实测覆盖率为 84.23%。
...
- [x] Run the current related tests with `COVERAGE_FILE=/tmp/quant-platform-contracts.coverage uv run --locked python -m pytest --cov=research_contracts --cov-report=term-missing -q tests/test_run_provenance.py tests/alpha/test_signal_artifact.py tests/test_positions_artifact.py tests/alpha/test_style_replica_signal_generator.py tests/test_backtest_bundle.py`.
- [x] Confirm the report names `.venv/lib/python3.13/site-packages/research_contracts` as the imported source and records the current 24% baseline.
- [x] Add `--cov=research_contracts` to the `coverage` case in `scripts/dev/run_tests.sh`, preserving the existing `--cov=packages` and `--cov=scripts` targets.
- [x] Re-run the exact command and confirm the output still reports the installed package, not an unexecuted source tree.
- [x] Commit the coverage target separately from tests.
...
- [x] Write tests for a valid aware-UTC clock, a valid non-UTC clock, naive timestamps, unsupported schema, and each violated ordering relation.
- [x] Write tests for the optional execution window being omitted, partially provided, ordered incorrectly, and required by `require_execution=True`.
- [x] Run `uv run --locked python -m pytest tests/contracts/test_research_clock.py -q` and confirm the tests exercise current public behavior.
- [x] Write envelope round-trip tests and reject wrong schema, missing required fields, and malformed payload types.
- [x] Run `uv run --locked python -m pytest tests/contracts/test_artifact_envelope.py -q`.
...
- [x] Use `tmp_path` to test file receipt creation, digest verification, missing files, and digest mismatch without repository data.
- [x] Use inline mappings to test ownership and run-manifest valid/invalid cases, including a clock failure propagated through a manifest.
- [x] Use temporary JSON files to test publication-manifest and lineage valid/invalid cases.
- [x] Run `uv run --locked python -m pytest tests/contracts/test_file_receipts.py tests/contracts/test_contract_ownership.py tests/contracts/test_research_run_manifest.py tests/contracts/test_publication_and_lineage.py -q`.
- [x] Add atomic-write failure coverage using a monkeypatched write/replace operation and assert the destination is absent or retains its prior complete contents.
...
- [x] Build minimal valid payload fixtures from the current exported schema and verify promotion checks accept them.
- [x] Remove or corrupt one required field per check and verify a precise rejection.
- [x] Cover A-share readiness positive and negative paths using generated evidence only.
- [x] Run `uv run --locked python -m pytest tests/contracts/test_promotion_evidence.py tests/contracts/test_a_share_readiness.py -q`.
...
- [x] Run `COVERAGE_FILE=/tmp/quant-platform-contracts.coverage uv run --locked python -m pytest --cov=research_contracts --cov-report=term-missing -q tests/contracts` and identify remaining uncovered statements.
- [x] Add direct tests for uncovered public validators that are used by current consumers; leave unreachable internal compatibility paths out of the threshold rationale only after checking imports across the workspace.
- [x] Add `--cov-fail-under=80` to the `coverage` case in `scripts/dev/run_tests.sh` after the measured result is at least 80%.
- [x] Run `uv run --locked python -m pytest tests/contracts -q` and the exact coverage command above.
- [x] Run `uv run --locked ruff check tests/contracts packages/research-contracts` and `uv run --locked ty check --error-on-warning packages/research-contracts tests/contracts`.
- [x] Commit the test suite and threshold together.
