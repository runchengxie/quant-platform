...
- [ ] Run `uv run --locked python -m pytest tests/test_execution_sim.py -q` and save the baseline output through pytest assertions rather than checked-in generated files.
- [ ] List functions and their callers with `rg -n '^def |^class |from .*execution_sim\.core|import .*execution_sim\.core' packages tests`.
- [ ] Add assertions for stable output columns, row ordering, and key numeric values to tests using existing small fixtures.
- [ ] Record `execution_sim.__init__.__all__`, all public names available from `core.py`, and every direct consumer import.
...
- [ ] Move only table/pre-tradability preparation helpers and their direct imports.
- [ ] Keep `core.py` names as imported re-exports and verify function signatures with `inspect.signature` assertions for public names.
- [ ] Patch `_build_execution_tables` at its new defining module in the test, then assert the public simulation entry invokes the patched function.
- [ ] Run `uv run --locked python -m pytest tests/test_execution_sim.py -q`.
...
- [ ] Move adjusted NAV planning, daily execution, ledger, and output helpers as one cohesive group.
- [ ] Move ideal NAV target, ledger, and daily result helpers as a separate cohesive group.
- [ ] Preserve target, feasible-plan, and realized-fill columns exactly.
- [ ] Run `uv run --locked python -m pytest tests/test_execution_sim.py tests/test_execution_sim_market_rules.py tests/test_execution_sim_quote_safety.py -q`.
...
- [ ] Move capacity execution and its private cohesive helpers.
- [ ] Keep `core.py` as re-exports and update `__init__.py` only when needed to preserve the same public surface.
- [ ] Compare sorted old/new public names with a test and import each public name through the package and compatibility module.
- [ ] Run `uv run --locked python -m pytest tests/test_execution_sim.py tests/test_execution_sim_market_rules.py tests/test_execution_sim_quote_safety.py -q`.
...
- [ ] Run `uv run --locked ruff check packages/portfolio-backtester/src/portfolio_backtester/execution_sim tests/test_execution_sim.py tests/test_execution_sim_market_rules.py tests/test_execution_sim_quote_safety.py` and fix each finding without broad rule suppressions.
- [ ] Run `uv run --locked ty check --error-on-warning packages/portfolio-backtester/src/portfolio_backtester/execution_sim tests/test_execution_sim.py tests/test_execution_sim_market_rules.py tests/test_execution_sim_quote_safety.py` and fix type issues at their source.
- [ ] Remove the relevant Ruff exclusion only when the full owning package is clean under the configured rules.
- [ ] Run the complete portfolio backtester unit test selection from CI.
- [ ] Commit code movement and static cleanup separately, with the golden behavior tests included before the moves.
