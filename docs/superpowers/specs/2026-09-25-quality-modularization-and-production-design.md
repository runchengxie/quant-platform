...
论文讨论奇异隐式系统时，先判断解是否存在、是否唯一，再讨论控制和估计。本项目只采纳这个检查顺序，不引入 descriptor-system 求解器，也不把矩阵秩直接解释成投资组合或策略的唯一性。

在 `quant-platform` 中，这个顺序对应以下现有边界：

1. `ResearchClock` 验证信息、信号、决策与执行时间的先后关系，供消费者检查时间因果性。
2. 组合优化先验证权重和暴露约束，并检查求解结果的约束残差。不可行的等权回退会明确失败原因。
3. 若多个解都满足约束，平台不凭约束矩阵的秩宣称解唯一。当前只提供调用者能使用的约束残差和失败信息，不增加未经需求验证的通用自由度或最优解稳定性 API。
4. 执行账本保留 `desired target -> rebalance plan -> order -> fill` 阶段，使策略目标、可执行计划和实际成交可以分别检查。

因子冗余、策略行为等价、潜在状态估计和更广泛的识别性分析属于研究消费者的问题。只有具体消费者证明需求后，才为其设计接口和验证方式。

### 实施状态

contracts 覆盖率、组合约束诊断测试、三个模块拆分和静态质量债务已分别通过 PR #47–55 完成，执行模拟测试拆分由 PR #56 完成。当前质量门禁与未完成的 production 发布以各计划页和发布清单为准。

原实施顺序如下，供核对 PR 历史和依赖关系：
