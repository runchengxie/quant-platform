...
`quant-platform` 是通用量化平台能力的权威仓库。公共框架迁移已经完成，`research-workspace` 和旧 platform submodule 仅保留作历史来源和兼容核对对象。

## 放置规则

| 内容 | 归属 |
| --- | --- |
| 数据接入、标准化、质量治理、版本和 published asset | 独立的 `quant-market-data-platform` |
...
| 报告、看板、Feishu 和运营交付 | `quant-intel-platform` 和 `quant-intel-deploy` |
