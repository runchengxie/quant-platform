...
def build_benchmark_series(
    ...
    if benchmark_return_series is not None and not benchmark_return_series.empty:
        returns_result = _benchmark_from_returns(benchmark_return_series, period_info)
        if returns_result is not None:
            return returns_result
    return _benchmark_from_prices(benchmark_df, entry_price_col, exit_price_col, period_info)


def _benchmark_from_returns(
    benchmark_return_series: pd.Series, period_info: list[dict[str, Any]]
) -> tuple[pd.Series, list[dict[str, Any]]] | None:
